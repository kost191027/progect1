...
#[tauri::command]
fn start_vpn(app: AppHandle, state: State<'_, AppState>, config_path: String) -> Result<(), String> {
        ...
        while let Some(event) = rx.recv().await {
            match event {
                CommandEvent::Stdout(line) => {
                    let _ = app_handle.emit("tunnel-log", String::from_utf8_lossy(&line).into_owned());
                }
